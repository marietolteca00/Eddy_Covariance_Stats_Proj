...
temp_levels <- c(50, 60, 70, 80)  # Pick readable bands
...
eto_pred <- pred_grid %>%
  mutate(
    ...
    lambda       = linkinv(logit_p),
    lambda_lwr   = linkinv(logit_p_lwr),
    lambda_upr   = linkinv(logit_p_upr)
  )

# Plot with CI
ggplot(eto_pred, aes(x = precip_in, y = lambda, color = factor(avg_air_temp_f))) +
  geom_ribbon(aes(ymin = lambda_lwr, ymax = lambda_upr, fill = factor(avg_air_temp_f)),
              alpha = 0.2, color = NA) +
  geom_line() +
  labs(
    x = "Daily Precipitation (inches)",
    y = "Predicted ETo (inches)",
    color = "Air Temp (°F)",
    fill = "Air Temp (°F)"
  ) +
  theme_light()
...
x2 <- runif(n, min = 2, max = 10)# avg temp # TEMP

# Defining the True Coefficients
sim_beta0 <- 1    # Intercept - # ETo
sim_beta1 <- 0.5  # Coefficient for x1 -  Precipitation decreases with Eto
sim_beta2 <- -0.2 # Coefficient for x2 -  Temperature increases with Eto
shape_param <- 2  # True shape parameter of the gamma distribution


# Calculate the expected mean of y based on predictors and coefficients
mu <- exp(sim_beta0 + sim_beta1 * x1 + sim_beta2 * x2)
...
# Simulate the Gamma-distributed outcome variable 'y'
...
sim_b0 <- sim_model$coefficients[1]
sim_b1 <- sim_model$coefficients[2]
sim_b2 <- sim_model$coefficients[3]
